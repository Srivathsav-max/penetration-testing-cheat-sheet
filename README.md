...
Everything was tested on Kali Linux v2022.1 (64-bit).
...
go install -v github.com/user/tool@latest
```
...
httpx -random-agent -json -o httpx_results.json -threads 100 -timeout 3 -l subdomains.txt -ports 80,443,8008,8080,8403,8443,9008,9080,9403,9443
```
...
for subdomain in $(cat subdomains_live.txt); do gau "${subdomain}"; done | sort -uf | tee gau_results.txt
```

Filter URLs from the results:

```bash
httpx -random-agent -json -o httpx_gau_results.json -threads 100 -timeout 3 -l gau_results.txt
